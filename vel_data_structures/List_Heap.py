...
def get_insertion_index(item, item_list: MutableSequence, reverse: bool = False):
	"""
	Returns the position where item should be inserted so that l stays sorted

	:param item: the item to insert :param l: the list where item could be inserted :return index: the index where
	item would be inserted
	:param item_list: the list where item could be inserted
	:param reverse: whether the index should insert the item into l in sorted order or reverse
	sorted order. Set to false by default
	"""
	...
	if reverse:
		...
		for i in range(len(item_list) - 1, -1, -1):
			if item_list[i] >= item >= item_list[i + 1]:
				return i + 1
	else:
		# Insert at the end
		if item < item_list[0]:
...
def insert_into_list(item, item_list: MutableSequence, reverse: bool = False):
	"""
	Inserts item into l in a position that keeps l sorted

	:param item: the item to insert :param l: the list where item will be inserted
	:param item_list: the list where item could be inserted
	:param reverse: whether we should
	insert the item into l in sorted order or reverse sorted order. Set to false by default
	"""
	item_list.insert(get_insertion_index(item, item_list, reverse), item)
...
class List_Heap(object):
	...
	We also have an option to turn the largest item if preferred
	"""
