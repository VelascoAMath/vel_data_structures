...
class _Node(object):
    ...
    def __repr__(self):
        return f"_Node(num_list={self.num_list.item_list} children={self.children})"
...
class BTree(object):
    ...
    def __init__(self, b=3, items=None, fast_insert=True):
    ...
    def _rebalance(self, visited_list):
        ...
        if visited_list[0] is not self._root:
            raise Exception(
                f"This {visited_list=} should have the root as the first element!"
            )
        ...
        while self.stack:
            ...
            if not added_child:
                if item_index < len(node.num_list):
                    ...
                    if level == 0:
                        level_to_width[level] = max(
                            level_to_width[level], len(str(item))
                        )
                    else:
                        level_to_width[level] = max(
                            len(str(item)) + 3 + level_to_width[level - 1],
                            level_to_width[level],
                        )
    ...
    def to_dot(self, f_name):
        ...
        g = graphviz.Digraph(filen, format='pdf')
        ...
        if self._root is not None:
            ...
            while stack:
                ...
                if node_id not in node_to_index:
                    ...
                    index += 1
                g.node(f'{node_to_index[node_id]}', f"{node.num_list.item_list}")
                ...
                for child in node.children:
                    ...
                    index += 1
                    g.edge(
                        f"{node_to_index[node_id]}", f"{node_to_index[child_node_id]}"
                    )
    ...
    def add_sorted(self, l):
        # Can only do this to an empty tree
        if len(self) > 0:
            self.add_items(l)
            return

        # Let's make sure the list is sorted
        l = sorted(l)
        ...
        while len(layer) > 1:
            parent = _Node()
            parent_layer = collections.deque()
            while layer:
                curr = layer.popleft()
                if layer:
                    if len(parent) >= self._b:
                ...
                # We're at the end
                    ...
                    if len(curr) < self._b:
                        pass
                    # Need to pass an item up
                    else:
                        sibling = _Node()
                        for _ in range((len(curr) - 1) // 2):
                            sibling.add(curr.pop())

                        while curr.children and sibling.is_under_fertile():
                            sibling._add_child(curr._pop_child())

                        parent.add(curr.pop())
                        parent._add_child(sibling)
        ...
        self._root = layer.pop()

        if len(self._root) >= self._b:
            curr = self._root
            sibling = _Node()
            parent = _Node()
            for _ in range((len(curr) - 1) // 2):
...
def main():
    ...
    def contains_test():
        SET_SIZE = 1000
        for n in tqdm([2, 3, 4, 5, 6, 7, 8, 9, 10, 15, 20], desc="Size loop"):
            for x in tqdm(range(100), desc="Contains"):
                random.seed(x)
                s = set(random.sample(list(range(SET_SIZE)), 100))
                c = set(range(SET_SIZE)) - s
